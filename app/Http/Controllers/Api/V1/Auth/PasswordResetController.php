    ...
    public function sendResetLink(PasswordResetRequest $request)
    {
        ...
        if ($user) {
            if (!$user->hasFilledProfile()) {
                return response()->json(['message' => __('common.PleaseLogInVIASocialNetwork')], Response::HTTP_UNPROCESSABLE_ENTITY);
            }
            Password::broker()->sendResetLink($request->only(['email']));
            return response()->json();
        }

        return response()->json(['message' => __('common.userNotFound')], Response::HTTP_UNPROCESSABLE_ENTITY);
    }
